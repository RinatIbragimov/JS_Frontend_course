...
function animate(draw, duration) {
	let start = performance.now(); // конкретное время в данный момент
	requestAnimationFrame(
		function animate(time) {
			let timePassed = time - start;
			if (timePassed > duration) {    // данный блок задает условия, когда 
					timePassed = duration;				// наша анимация должна выполняться
			}
			draw(timePassed);
